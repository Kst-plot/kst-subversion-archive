...
/** frames to read in one step */
#define KV_MAXREAD 1000

/** Create a KstRVector: raw data from a file */
KstRVector::KstRVector(KstFilePtr in_file, const QString &in_field,
...
: KstVector() {
  ...
  while( !n.isNull() ) {
	...
	if (o_file == "|") {
	  in_file = *KST::fileList.findFileName(e.text());
	} else {
	  in_file = *KST::fileList.findFileName(o_file);
	}
...
void KstRVector::commonRVConstructor(KstFilePtr in_file,
...
void KstRVector::change(KstFilePtr in_file, const QString &in_field,
                        const QString &in_tag,
                        int in_f0, int in_n,
...
void KstRVector::changeFile(KstFilePtr in_file) {
  File = in_file;
  reset();
}
...
void KstRVector::save(QTextStream &ts) {
  ...
  ts << "  <filename>" << File->fileName() << "</filename>" << endl;
...
QString KstRVector::getFilename() const {
...
QString KstRVector::label() const {
  ...
  Field.toInt(&ok);
  if (ok && File->fileType() == i18n("ASCII")) {
...
QString KstRVector::fileLabel() const {
  return File->fileName();
}

void KstRVector::reset() {
  SPF = File->samplesPerFrame(Field.latin1());	// update SPF
  F0 = NF = 0;
  resize(0);
  _numSamples = 0;
  _dirty = true;
...
void KstRVector::checkIntegrity() {
  ...
  if (SPF != File->samplesPerFrame(Field.latin1()) ||
      File->numFrames() < NF) {
    reset();
  }
...
KstObject::UpdateType KstRVector::update(int update_counter) {
  ...
  if (ReqNF < 1) { // read to end of file
    new_f0 = ReqF0;
    new_nf = File->numFrames() - new_f0;
  } else if (ReqF0 < 0) { // count back from end of file
    new_nf = File->numFrames();
    if (new_nf > ReqNF) new_nf = ReqNF;
    new_f0 = File->numFrames() - new_nf;
  } else {
    new_f0 = ReqF0;
    new_nf = ReqNF;
    if (new_f0 + new_nf > File->numFrames() )
      new_nf = File->numFrames() - new_f0;
    if (new_nf <= 0) {
      new_nf = new_f0 = 0;
    }
  ...
  if (DoSkip) {
	...
	ave_nread =
          File->readField(AveReadBuf, Field.latin1(), new_f0+i, Skip);
	...
	n_read += File->readField(_v + _numSamples + j,
                                  Field.latin1(), new_f0 + i, -1);
      }
    }
  } else {
    /**** Reallocate V if necessary ****/
    ...
    if (NF>0) NF--; /* last frame read was only partially read... */

    /**** Read the new data from File ****/
    n_read = File->readField(_v+NF*SPF, Field.latin1(), new_f0 + NF,
                             new_nf - NF - 1);
    n_read += File->readField(_v+(new_nf-1)*SPF, Field.latin1(),
                              new_f0 + new_nf-1, -1);
  }
...
bool KstRVector::isValid() const {
  return File->isValidField(Field);
}

int KstRVector::getFileLength() const {
  return File->numFrames();
}
