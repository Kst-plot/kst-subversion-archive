...
inline void KstPlot::commonConstructor(const QString &in_tag,
  ...
  if (XMax<=XMin) { // not OK: ignore request
    XMin = 0; XMax = 1;
    if (XScaleMode!=AUTOUP) XScaleMode = AUTO;
  }
  if (YMax<=YMin) {
    YMin=0; YMax = 1;
    if (YScaleMode!=AUTOUP) YScaleMode = AUTO;
  }
...
void setTickPix(double &xtickpix, double &ytickpix, int x_pix, int y_pix) {
  ...
  if (xtickpix < 2.0) xtickpix = 2.0; // but at least 2 pixels
  ytickpix = xtickpix;
}
...
void KstPlot::setBorders(double &xleft_bdr_px, double &xright_bdr_px,
  ...
  // FIXME: loop is not needed for this calculation
  for (i = 0; i*YTick + Yorg < YMax; i++)
     /* do nothing */;
  i--;
...
void KstPlot::paint(QPainter &p, double in_xleft_bdr_px) {
  ...
  QRect v = p.window();
  p.fillRect(v, backgroundColor());
  x_px = v.width();
  y_px = v.height();

  penWidth = x_px/999;
  if (penWidth == 1) penWidth = 0;

  p.setPen(QPen(foregroundColor(), penWidth));
  ...
  if (x_px-xright_bdr_px - xleft_bdr_px < 10) return;
  if (y_px-ybot_bdr_px-ytop_bdr_px+1.0 - ytop_bdr_px < 10) return;
  ...
  RelWinRegion.setRect((int)0, (int)0, (int)x_px, (int)y_px);
...
#undef isnan
	  ...
	  last_y1=Y1;
          if (nopoint) {
            if (overlap) {
              if ((X2>=Lx) && (X2<=Hx)) {
                if ((maxY>Hy)&&(minY<=Hy)) maxY=Hy;
                if ((minY<Ly)&&(maxY>=Ly)) minY=Ly;
