...
#define CATEGORIESIMAGEPROPERTY_DEBUG 1
...
{
#if CATEGORIESIMAGEPROPERTY_DEBUG > 0
	MYDEBUG<<endl;
#endif
    m_p_imageEntry = imageEntry;
    m_p_categoryDBManager = categoryDBManager;
    m_numerOfImages = 1;
 		    ...
 		    m_numerOfImages(numerOfImages)
{
	...
	MYDEBUG<<"BEGIN"<<endl;
#endif
	m_p_numberOfImagesForCategory = new QDict<QVariant>(200);
	...
	if(m_p_categoryDBManager)
	{
		QStringList* l_p_cat_id_list = m_p_categoryDBManager->getCategoryIdListImage(image_id_list, false);
		if(l_p_cat_id_list)
		{
			for ( QStringList::Iterator it = l_p_cat_id_list->begin(); it != l_p_cat_id_list->end(); ++it )
			{
				QVariant* num = (*m_p_numberOfImagesForCategory)[*it];
				if(!num)
				{
					m_p_numberOfImagesForCategory->insert(*it, new QVariant(1));
				}
				else
				{
					m_p_numberOfImagesForCategory->replace(*it, new QVariant(num->toInt()+1));
				}
			}
			delete(l_p_cat_id_list);
			l_p_cat_id_list=0;
		}
	}
	if(!imageEntryList.isEmpty())
		m_p_imageEntry = imageEntryList.first();
	...
	//--------------------------------------------------------------------------
	if(m_numerOfImages == 1 && imageEntryList.first())
		imagenameTextLabel->setText(i18n("File: %1").arg(imageEntryList.first()->getName()));
	else
...
CategoriesImageProperty::~CategoriesImageProperty()
{
    ...
    delete (m_p_numberOfImagesForCategory);
}
...
CategoriesImageProperty::init()
{
	...
	if(!m_p_imageEntry) return;
	beginDateWidget->setDate(m_p_imageEntry->getDate_begin().date());
	endDateWidget->setDate(m_p_imageEntry->getDate_end().date());
	imagenameTextLabel->setText( m_p_imageEntry->getName() );
	commentTextEdit->setText( m_p_imageEntry->getComment() );
	noteSlider->setValue(m_p_imageEntry->getNote());
}
...
CategoriesImageProperty::visitCategoryTree()
{
	CategoriesImagePropertyCategoryItem *root =
		dynamic_cast<CategoriesImagePropertyCategoryItem *>(categoriesListView->firstChild()->firstChild());
	visitCategoryTree_rec(root, 0);

	while(root)
	{
		root->setOpen(root->getSubCategoryIsSelected());
		root = dynamic_cast<CategoriesImagePropertyCategoryItem *>(root->nextSibling ());
	}
}
...
CategoriesImageProperty::visitCategoryTree_rec(CategoriesImagePropertyCategoryItem *root, int d)
{
	...
	while(root)
	{
		CategoriesImagePropertyCategoryItem *item =
			dynamic_cast<CategoriesImagePropertyCategoryItem *>(root->firstChild());
		...
		while(item)
		{
			...
			item =
				dynamic_cast<CategoriesImagePropertyCategoryItem *>(item->nextSibling ());
		}
		...
		root =
			dynamic_cast<CategoriesImagePropertyCategoryItem *>(root->nextSibling());
	}
...
{
	QPtrList<CategoryNode> cat_list = m_p_categoryDBManager->getRootCategories();
	...
	for(CategoryNode *node = cat_list.first(); node; node=cat_list.next())
	{
		...
		QVariant* num = (*m_p_numberOfImagesForCategory)[item->getId()];
		if(num)
		{
...
CategoriesImageProperty::getCheckedCategories(bool takeIntoAccountTristate)
{
	...
	CategoriesImagePropertyCategoryItem *item =
		dynamic_cast<CategoriesImagePropertyCategoryItem *>(categoriesListView->firstChild()->itemBelow());
	while(item)
	{
		item->setOpen(true);
		if(
		...
		item =
			dynamic_cast<CategoriesImagePropertyCategoryItem *>(item->itemBelow());
	}
...
CategoriesImageProperty::getRemovedCategories()
{
	...
	QDictIterator<QVariant> it (*m_p_numberOfImagesForCategory);
	for ( ; it.current(); ++it )
	{
