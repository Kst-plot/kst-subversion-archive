...
void CHexBuffer::cursorStep( uint size, bool forward, bool modulo )
{
  if( forward == true )
  {
    if( modulo == true )
...
//gbertani virtualized for serlook purposes
void CHexBuffer::cursorRight( bool cellLevel )
{
...
#if 0 // gbertani serlook - generate lots of "out of range" messages, bookmarks not yet used
  //
...
int CHexBuffer::print( CHexPrinter &printer, CProgress &p )
{
  ...
  while( remaining > 0 )
  {
    ...
    for( uint i=0; i < lineInPage; i++, line++ )
    {
      ...
      y += lineHeight();// - mLayout.horzGridWidth;
