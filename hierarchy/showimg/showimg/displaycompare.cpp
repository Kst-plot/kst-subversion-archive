...
{
	QCheckListItem* l_p_item=dynamic_cast<QCheckListItem*>(listEq->firstChild ());
	QCheckListItem* itemTmp=NULL;
	while(l_p_item)
	{
		if(l_p_item->isOn ())
		{
			itemTmp=dynamic_cast<QCheckListItem*>(l_p_item->nextSibling());
			QFile::remove (l_p_item->text(0) );
			listEq->takeItem (l_p_item);

			l_p_item=itemTmp;
		}
		else
			l_p_item=dynamic_cast<QCheckListItem*>(l_p_item->nextSibling());
	}

	///
	for (l_p_item = dynamic_cast<QCheckListItem*>(listName->firstChild ()); 
		 l_p_item; 
		 l_p_item=dynamic_cast<QCheckListItem*>(l_p_item->nextSibling()) )
	{
		if(l_p_item->isOn ())
...
DisplayCompare::slotDisplayLeft(QListViewItem * item)
{
	...
	listEq->clear();
	QPtrVector < QFile > *l_p_list = (QPtrVector < QFile > * )cmp->find(item->text(0));
	...
	QCheckListItem *it=NULL, *last=NULL;
	QFile *l_p_file=NULL;
	QFileInfo *l_p_fi=new QFileInfo();
	QString fn;

	for (unsigned int i = 0; i < l_p_list->size (); i++)
	{
	      l_p_file=dynamic_cast<QFile*>(l_p_list->at(i));
	      l_p_fi->setFile(*l_p_file);
	      fn =  l_p_fi->absFilePath ();
	      if(l_p_fi->exists ())
	      {
