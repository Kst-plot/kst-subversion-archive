...
////
    ...
    m_p_dateFormatLine = new KLineEdit( dateFormatOption, "m_p_dateFormatLine" );
    dateFormatOptionLayout->addWidget( m_p_dateFormatLine );
...
void
DateTimeOption::setDateFormat(const QString & format)
{
	m_p_dateFormatLine->setText(format);
}
QString
DateTimeOption::getDateFormat()
{
	return m_p_dateFormatLine->text();
}
void
DateTimeOption::setTimeFormat(const QString & format)
{
...
{
	setDateFormat( KGlobal::locale()->dateFormatShort().replace("/"," "));
	setTimeFormat( KGlobal::locale()->timeFormat().replace("/"," "));
}
void
DateTimeOption::slotOk()
{
	if(getTimeFormat().find("/")>-1)
	{
		KMessageBox::sorry( this, i18n("Time pattern cannot contain '/'.") );
		return ;
	}
	else
	if(getDateFormat().find("/")>-1)
	{
		KMessageBox::sorry( this, i18n("Date pattern cannot contain '/'.") );
		return ;
	}
	KDialogBase::slotOk();
}
...
////
	...
	m_progressDialog = new KProgressDialog( parentWidget(), "renamed ProgressDialog", i18n("Renaming...") );
...
QDict<QString>
RenameSeries::getRenamedFiles()
{
	return m_batchRenamer->getRenamedFiles();
}
...
bool RenameSeries::checkErrors(bool checkDir)
{
	...
	if( paternLineEdit->text().find("/")>-1 ) {
		KMessageBox::sorry( this, i18n("Pattern cannot contain '/'.") );
		return false;
	}
	else
...
RenameSeries::slotOk()
{
	...
	m_progressDialog->progressBar()->setTotalSteps(arrayNames.size());
	m_progressDialog->progressBar()->setFormat(i18n("Renaming %n file...", "Renaming %n files...", arrayNames.size()));
...
QSize
RenameSeries::sizeHint () const
{
	return QSize(348, 170);
}

#include "renameseries.moc"
