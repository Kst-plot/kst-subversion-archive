...
BatchRenamer::BatchRenamer(KProgressDialog *p)
{
    ...
    KService* l_p_service=NULL;
    KService::List l_list = KService::allServices();
    for( unsigned int i = 0; i < l_list.count(); i++ ) 
	{
        KService* s = (KService*)l_list[i];
        //KService* s = dynamic_cast<KService*>(&l_list[i]);
        ...
        if( !s->terminal() && s->type() == QString::fromLatin1("Service") && s->hasServiceType( "KFilePlugin" ) )
	{
		...
		)
	    {
	     ...
	     QStringList options = l_p_service->serviceTypes();
...
BatchRenamer::getKeys()
{
    ...
    for( unsigned int i = 0; i < keys.count(); i++ )
    {
        l_list.append(keys[i].right(keys[i].length()-getPattern().length()));
    }
    l_list.sort();
    return l_list;
}
