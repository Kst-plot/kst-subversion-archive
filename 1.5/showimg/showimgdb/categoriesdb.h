...
public:
	...
	void addLink(const KURL::List& l_list, int cat_id);

	void addNote(const KURL::List& l_list, int note);
...
private:
	QPtrList<CategoryNode> m_rootCat_list;
	QPtrVector<CategoryNode> m_catnode_list;

	Categories *m_p_categories;

	QDict<QVariant> *m_p_path_dict;
	bool m_useCache;


	QPtrList <QPtrList <QVariant> > *m_p_cachedImageToAdd;
};
