...
FrameSource::FrameSource(const QString& filename, const QString& type)
...
void FrameSource::save(QTextStream &ts) {
...
KstDataSource *create_frame(const QString& filename, const QString& type) {
  return new FrameSource(filename, type);
}
...
bool understands_frame(const QString& filename) {
  int err = 0;
