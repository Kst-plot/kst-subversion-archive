...
void KstView::paintEvent(QPaintEvent *) {
  ...
  if (qpixmap == 0L) {
    ...
    qpixmap->fill(KstSettings::globalSettings()->backgroundColor);
    erase();
    return;
  }
...
void KstView::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (e->state() & Qt::LeftButton &&
    ...
    QDragObject *d = new KstLabelDrag(this, plot_num, i_label, _draggablePoint - plot->GetWinRegion().topLeft() - rectBounding.topLeft(), pm);
    d->dragMove();
    _draggableLabel = -1;
    _draggablePlot = -1;
    return;
        ...
        MouseInfo->getMode() == LABEL_TOOL &&
        MouseInfo->getLegend(e) &&
        plot_num == _draggablePlot )
  {
    // Start a drag
    KstPlot *plot = KST::plotList.at(plot_num);
    KstLegend *legend = plot->Legend;
    ...
    QDragObject *d = new KstLegendDrag(this, plot_num, _draggablePoint - plot->GetWinRegion().topLeft() - rectBounding.topLeft(), pm);
    d->dragMove();
    _draggableLabel = -1;
    _draggablePlot = -1;
    return;
    ...
    for (i = i0; i < iN; i++) {
      ...
      if (tie_rect.contains(e->pos())) {
        ...
        if(_draggableLabel >= 0 && MouseInfo->getMode() == LABEL_TOOL) {
          _draggablePlot = i;
        } else if(_draggableLabel == -1 && MouseInfo->getLegend(e)) {
          _draggablePlot = i;
        } else {
