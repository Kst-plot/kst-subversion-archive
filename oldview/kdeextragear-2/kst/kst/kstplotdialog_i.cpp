...
void KstPlotDialogI::applyLabels(KstPlot *plot) {
  ...
  int i, i_plot, plot_min, plot_max;
  ...
  /* legend */
  if (applyLegendSettingsToAll->isChecked()) {
    i_plot = plot_min = 0;
    plot_max = KST::plotList.count() - 1;
  } else {
    plot_min = plot_max = i_plot = Select->currentItem();
  ...
  strLayout = LegendPosition->currentText();
  for( i=0; i<iNumLegendLayouts; i++) {
    if( strLayout.compare( LegendLayouts[i].Label ) == 0 ) {
      layout = LegendLayouts[i].type;
      break;
    }
  }

  strAlignment = LegendAlignment->currentText();
  for( i=0; i<iNumLegendAlignments; i++) {
    if( strAlignment.compare( LegendAlignments[i].Label ) == 0 ) {
...
void KstPlotDialogI::updateSampLegendLabel() {
  if (LegendFontComboBox->currentText() != 0) {
