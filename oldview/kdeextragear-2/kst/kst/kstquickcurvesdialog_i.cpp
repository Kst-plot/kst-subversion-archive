...
void KstQuickCurvesDialogI::update() {
  ...
  QString u;
  if (j >= 0) {
    u = KST::dataSourceList[j]->fileName();
    KST::dataSourceList.lock().readUnlock(); // avoid deadlock due to signal
    FileName->setURL(u);
  } else {
...
bool KstQuickCurvesDialogI::apply(bool autolabel) {
  ...
  if (x_is_new) {
    ...
    vx = new KstRVector(file, XAxis->currentText(),
			 ...
			 DoSkip->isChecked(),
			 DoFilter->isChecked());
    KST::addVectorToList(KstVectorPtr(vx));
  }
  ...
  if (y_is_new) {
    ...
    vy = new KstRVector(file, YAxis->currentText(),
			 ...
			 DoSkip->isChecked(),
			 DoFilter->isChecked());
    KST::addVectorToList(KstVectorPtr(vy));
  }
