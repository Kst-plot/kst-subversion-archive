	...
	double outScalars[], int iNumParams)
{
  ...
  if (inArrayLens[YVALUES] >= 2 && inArrayLens[XVALUES] >= 2) {
    ...
    if( inArrayLens[WEIGHTS] < iLength ) {
      iLength = inArrayLens[WEIGHTS];
    }
                ...
                if( pWork != NULL ) {
                  ...
                  if( pVectorWeights != NULL ) {
                    ...
                    for( i=0; i<iLength; i++ ) {
                      ...
                      gsl_vector_set( pVectorWeights, i, inArrays[WEIGHTS][i] );
                      for( j=0; j<iNumParams; j++ ) {
                        dX = calculate_matrix_entry( inArrays[XVALUES][i], j );
