...
void DataMatrix::changeFrames(int xStart, int yStart,
  ...
  commonConstructor(this->dataSource(), _field, xStart, yStart, xNumSteps, yNumSteps, doAve, doSkip, skip, minX, minY, stepX, stepY);
}
...
QString DataMatrix::filename() const {
  if (this->dataSource()) {
    return QString(this->dataSource()->fileName());
  }
  return QString::null;
}
*/
...
QString DataMatrix::label() const {
  ...
  _field.toInt(&ok);
  if (ok && this->dataSource()) {
    this->dataSource()->readLock();
    if (this->dataSource()->fileType() == "ASCII") {
...
bool DataMatrix::isValid() const {
  if (this->dataSource()) {
    this->dataSource()->readLock();
    bool fieldValid = this->dataSource()->matrix().isValid(_field);
    this->dataSource()->unlock();
    return fieldValid;
  }
  return false;
...
bool DataMatrix::checkValidity(const DataSourcePtr ds) const {
  if (ds) {
    ds->readLock();
    bool rc = ds->matrix().isValid(this->_field);
    ds->unlock();
    return rc;
  }
  return false;
...
qint64 DataMatrix::minInputSerialOfLastChange() const {
  if (this->dataSource()) {
    return (this->dataSource()->serialOfLastChange());
  }
  return LLONG_MAX;
}
...
void DataMatrix::_resetFieldStrings() {
  const QMap<QString, QString> meta_strings = this->dataSource()->matrix().metaStrings(_field);
...
void DataMatrix::internalUpdate() {
  ...
  const DataInfo info = this->dataSource()->matrix().dataInfo(_field);
...
PrimitivePtr DataMatrix::makeDuplicate() const {
  ...
  matrix->writeLock();
  matrix->change(this->dataSource(), _field, _reqXStart, _reqYStart, _reqNX, _reqNY, _doAve, _doSkip, _skip, _minX, _minY, _stepX, _stepY);
...
QString DataMatrix::descriptionTip() const {
  return i18n(
      ...
      "  Field: %3\n"
      "  %4 x %5"
      ).arg(Name()).arg(this->dataSource()->fileName()).arg(field()).arg(_nX).arg(_nY);
}

QString DataMatrix::propertyString() const {
  return i18n("%1 of %2").arg(field()).arg(this->dataSource()->fileName());
}
...
int DataMatrix::readMatrix(MatrixData* data, const QString& matrix, int xStart, int yStart, int xNumSteps, int yNumSteps, int skip)
{
  ...
  return this->dataSource()->matrix().read(matrix, p);
}
