...
int kstfit_nonlinear(
  ...
  if( inArrayLens[WEIGHT] < iLengthData ) {
    iLengthData = inArrayLens[WEIGHT];
  }
    ...
    if( pResult[0] != NULL && 
      ...
      if( pSolver != NULL ) {
        ...
        if( pMatrixCovariance != NULL ) {
          function_initial_estimate( inArrays[X], inArrays[Y], iLengthData, dXInitial );
          ...
          for( i=0; i<iLengthData; i++ ) {
            outArrays[0][i] = function_calculate( inArrays[0][i], dXInitial );
