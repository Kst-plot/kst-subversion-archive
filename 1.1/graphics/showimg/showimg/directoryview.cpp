...
{
	aDirCopy	=new KAction(i18n("Copy Folder To..."), "editcopy", 0, this, SLOT(slotDirCopy()), actionCollection, "editdircopy");
	aDirMove	=new KAction(i18n("Move Folder To..."), 0, this, SLOT(slotDirMove()),actionCollection , "editdirmove");

	aDirMoveToLast=new KAction(i18n("Move Folder to Last Directory"), 0,
								this, SLOT(slotDirMoveToLast()),actionCollection , "moveDirToLast");
	aDirCopyToLast=new KAction(i18n("Copy Folder to Last Directory"), 0,
								this, SLOT(slotDirCopyToLast()),actionCollection , "copyDirToLast");

	//aDirPaste	=new KAction(i18n("Paste"), "editpaste", 0, this, SLOT(slotDirPaste()),actionCollection , "editdirpaste");
	aDirPasteFiles	=new KAction(i18n("Paste Files"), "editpaste", 0, this, SLOT(slotDirPasteFiles()),actionCollection , "editdirpaste files");
	aDirRecOpen	=new KAction(i18n("Recursively Open"), 0, this, SLOT(recursivelyOpen()), actionCollection , "dirRecOpen");

	aDirRename=new KAction(i18n("&Rename Item..."), "item_rename", 0, this, SLOT(slotRename()), actionCollection , "editdirrename");

	aDirTrash=new KAction(i18n("&Move Item to Trash"), "edittrash", 0, this, SLOT(slotTrash()), actionCollection , "editdirtrash");
	aDirDelete=new KAction(i18n("&Delete"), "editdelete", 0, this, SLOT(slotSuppr()), actionCollection , "editdirdelete");

	aPreviousDir =new KAction(i18n("Previous Directory"), "1leftarrow", KShortcut(SHIFT+Key_Space), this, SLOT(goToPreviousDir()),actionCollection, "Previous Directory" );
	aNextDir = new KAction(i18n("Next Directory"), "1rightarrow", KShortcut(CTRL+Key_Space), this, SLOT(goToNextDir()), actionCollection, "Next Directory");

	aDirInfo =new KAction(i18n("Describe Directory..."), 0, this, SLOT(slotDirInfo()), actionCollection, "Dir Info");
	aDirProperties =new KAction(i18n("Properties"), "info", 0, this, SLOT(slotDirProperty()), actionCollection, "Dir Properties");

	aDirNewFolder =new KAction(i18n("New Directory..."), "folder_new", 0, this, SLOT(slotNewDir()), actionCollection, "editdirnew");
	aDirNewAlbum =new KAction(i18n("New Album..."), "txt", 0, this, SLOT(slotNewAlbum()), actionCollection, "editalbumnew");
...
DirectoryView::initMenu(KActionCollection *actionCollection)
{
	...
	actionNewItems = new KActionMenu( i18n("Create &New"), "filenew", actionCollection, "dirview create_new_items" );
	...
	aCopyActions = new KActionMenu( i18n("Copy"), 0, actionCollection, "Copy Folders actions" );
	...
	aMoveActions = new KActionMenu( i18n("Move"), 0, actionCollection, "Move Folders actions" );
...
DirectoryView::updateActions(ListItem *item)
{
	...
	{
		if(!(item->getType () == "Directory" || item->getType () == "Album")) enableAction=false;
	}
...
*/
		MYDEBUG<<"Deleting directory " << dirfullname<<endl;
	...
	{
		MYDEBUG<<"Can't find directory " << dirfullname<<endl;
	}
}
...
DirectoryView::slotNewAlbum (ListItem *item)
{
	...
	url = KURL("file:"+item->fullName()+"/"+newName+".sia");
...
DirectoryView::slotNewDir (ListItem *item)
{
	...
	url = KURL("file:"+((Directory*)item)->fullName()+"/"+newName);
	while(ok && !newName.isEmpty() && QFileInfo(url.path()).exists())
	{
...
/*	KIO::FileCopyJob *job = KIO::file_move(urlorg, urldest, -1, true, false, true);
	connect(job, SIGNAL(result( KIO::Job *)),
			this, SLOT(movingDirDone( KIO::Job *)));*/
...
bool
DirectoryView::loadThumbnails()
{
	return m_loadThumbnails;
}

void
DirectoryView::setLoadThumbnails(bool load)
{
...
DirectoryView::updateDestDirTitle(const QString& dir)
{
	...
	void DirectoryView::dirListerClear(){MYDEBUG<<endl;}
	void DirectoryView::dirListerNewItems(const KFileItemList&){MYDEBUG<<endl;}
	void DirectoryView::dirListerDeleteItem(KFileItem*){MYDEBUG<<endl;}
	void DirectoryView::dirListerRefreshItems(const KFileItemList&){MYDEBUG<<endl;}
	void DirectoryView::dirListerStarted(){MYDEBUG<<endl;}
	void DirectoryView::dirListerCompleted(){MYDEBUG<<endl;}
	void DirectoryView::dirListerCanceled(){MYDEBUG<<endl;}
	void DirectoryView::completed(){MYDEBUG<<endl;}
	void DirectoryView::canceled(){MYDEBUG<<endl;}
