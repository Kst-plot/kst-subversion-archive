...
class CHexBuffer : public QByteArray
{
  ...
  public:
    ...
    //gbertani: virtualized
    virtual bool inputAtCursor( QChar c );
    virtual int  inputAtCursor( const QByteArray &buf, uint oldSize );
    ...
    //gbertani virtualized for serlook purposes
    //inline void cursorRight( bool cellLevel );
    virtual void cursorRight( bool cellLevel );
    
    inline void cursorLeft( bool cellLevel );
    
    //gbertani virtualized for serlook purposes
    //inline void cursorStep( uint size, bool forward, bool modulo );
    virtual void cursorStep( uint size, bool forward, bool modulo );
...
//gmbertani protecting all instead of private 
//for serlook's purposes
...
signals:
  //void fileSize( uint size ); NOT USED (gmbertani 28/5/03)
};
...
inline const QColor &CHexBuffer::foregroundColor( uint column )  
{
  ...
  {
    return( mColor.primaryFg[ (uchar)mColorIndex[ column ] ] );
  }
}
