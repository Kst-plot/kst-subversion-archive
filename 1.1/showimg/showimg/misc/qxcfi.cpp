...
void XCFImageFormat::readXCF ( QImageIO* image_io )
{
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on header tag"<< endl;
    return;
  }
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on image info" << endl;
    return;
  }
  ...
  while ( true ) {
    ...
    if ( xcf_io.failed() ) {
      kdDebug() << "XCF: read failure on layer offsets"<< endl;
      return;
    }
  ...
  if ( !xcf_image.initialized ) {
    kdDebug() << "XCF: no visible layers!"<< endl;
    return;
  }
					   ...
					   XCFImage& xcf_image )
{
  while ( true ) {
    ...
    if ( !loadProperty( xcf_io, type, bytes ) ) {
      kdDebug() << "XCF: error loading global image properties"<< endl;
      return false;
    }
    ...
    default:
      kdDebug() << "XCF: unimplemented image property d, size d"<< type<<" "<< bytes.size()<< endl;
    }
  }
}
...
bool XCFImageFormat::loadLayer ( SafeDataStream& xcf_io, XCFImage& xcf_image )
{
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on layer"<< endl;
    return false;
  }
  ...
  if ( xcf_io.failed() ) {
    kdDebug() <<"XCF: read failure on layer image offsets" << endl;
    return false;
  }
...
bool XCFImageFormat::loadLayerProperties ( SafeDataStream& xcf_io, Layer& layer )
{
  while ( true ) {
    ...
    if ( !loadProperty( xcf_io, type, bytes ) ) {
      kdDebug() <<  "XCF: error loading layer properties"<< endl;
      return false;
    }
    ...
    default:
      kdDebug() << "XCF: unimplemented layer property d, size d"<< type<<" "<< bytes.size()<< endl;
    }
  }
}
...
bool XCFImageFormat::loadChannelProperties ( SafeDataStream& xcf_io, Layer& layer )
{
  while ( true ) {
    ...
    if ( !loadProperty( xcf_io, type, bytes ) ) {
      kdDebug() << "XCF: error loading channel properties" << endl;
      return false;
    }
    ...
    default:
      kdDebug() << "XCF: unimplemented channel property d, size d"<< type<<" "<< bytes.size()<< endl;
    }
  }
}
...
bool XCFImageFormat::loadHierarchy ( SafeDataStream& xcf_io, Layer& layer )
{
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on layer s image header "<< layer.name << endl;
    return false;
  }
  ...
  do {
    ...
    if ( xcf_io.failed() ) {
      kdDebug() << "XCF: read failure on layer s level offsets "<< layer.name << endl;
      return false;
    }
  } while ( junk != 0 );
...
bool XCFImageFormat::loadLevel ( SafeDataStream& xcf_io, Layer& layer, Q_INT32 bpp )
{
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on layer %s level info "<< layer.name << endl;
    return false;
  }
  ...
  for ( uint j = 0; j < layer.nrows; j++ ) {
    for ( uint i = 0; i < layer.ncols; i++ ) {
      ...
      if ( offset == 0 ) {
	kdDebug() << "XCF: incorrect number of tiles in layer " << layer.name << endl;
	return false;
      }
      ...
      if ( xcf_io.failed() ) {
	kdDebug() << "XCF: read failure on layer s level offset look-ahead " << layer.name << endl;
	return false;
      }
	...
	return false;
      ...
      if ( xcf_io.failed() ) {
	kdDebug() << "XCF: read failure on layer s level offset "<< layer.name<< endl;
	return false;
      }
    }
  }
...
bool XCFImageFormat::loadMask ( SafeDataStream& xcf_io, Layer& layer )
{
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on mask info"<< endl;
    return false;
  }
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on mask image offset" << endl;
    return false;
  }
				   ...
				   int data_length, Q_INT32 bpp )
{
  ...
  if ( read_length<=0 ) {
    delete[] xcfodata;
    kdDebug() <<  "XCF: read failure on tile" << endl;
    return false;
  }
 ...
 bogus_rle:
  ...
  kdDebug() << "The run length encoding could not be decoded properly"<< endl;
  delete[] xcfodata;
  return false;
}
				    ...
				    QByteArray& bytes )
{
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on property type"<< endl;
    return false;
  }
  ...
  if ( type == PROP_COLORMAP ) {
    ...
    if ( xcf_io.failed() ) {
      kdDebug() << "XCF: read failure on property d size "<< type  << endl;
      return false;
    }
  ...
  // The USER UNIT property size is not correct. I'm not sure why, though.
    ...
    if ( xcf_io.failed() ) {
      kdDebug() <<  "XCF: read failure on property d "<< type << endl;
      return false;
    }
  ...
  if ( xcf_io.failed() ) {
    kdDebug() << "XCF: read failure on property d data, size d"<< type <<" "<< size << endl;
    return false;
  }
