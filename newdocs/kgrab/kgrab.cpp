...
KGrab::createMenu()
{
    ...
    KAction *newAct = new KAction( KIcon("filenew"), i18n("&New Snapshot"), 
                                   actionCollection(), "new_snapshot" );
    ...
    KStdAction::saveAs( this, SLOT( slotSaveAs() ), actionCollection() , "file_saveas");

    KAction *owAct = new KAction( KIcon("project_open"), i18n("Open &With..."), 
                                  actionCollection(), "file_openwith");
    ...
    KStdAction::quit( this, SLOT( exit() ), actionCollection() , "file_quit");

   
    // Edit Menu

    copyAct = new KAction( KIcon("editcopy"), i18n("&Copy"), actionCollection(), "edit_copy" );
...
bool KGrab::save( const KUrl& url )
{
    if ( KIO::NetAccess::exists( url, false, this ) ) {
        ...
        if (KMessageBox::Continue != KMessageBox::warningContinueCancel( this, text, title, 					     KStdGuiItem::cont(), i18n("Overwrite") ) )
        {
            return false;
        }
    }
...
void KGrab::slotOpen()
{
    if(!save(filename.url()))
        return;
    OpenWith* ow = new OpenWith(QString("image/png"), filename.url(), this);
    ow->exec();
}
...
void KGrab::slotModeChanged( int mode )
{
    switch ( mode )
    {
    case 0:
