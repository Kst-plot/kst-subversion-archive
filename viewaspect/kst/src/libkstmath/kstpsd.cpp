...
void KstPSD::createWindow( int apodizeFxn, double *w, int len, double gaussianSigma) {
  double a = double(len/2);
  double x;
  double sW = 0.0;
  int i;
  ...
  switch (apodizeFxn) {
    ...
    case 0: 
      for (i = 0; i < len; i++) {
        w[i] = (1.0 - cos(2.0*M_PI*double(i)/double(len)));
        sW += w[i] * w[i];
      }
...
void KstPSD::generateWindow(int len) {
  if (len != _WLen || _prevApodizeFxn != _apodizeFxn) {   
    createWindow(_apodizeFxn, _w, len, _gaussianSigma);
