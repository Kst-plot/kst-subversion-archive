...
void KstImageDialogI::fillFieldsForEditNoUpdate() {
  ...
  palList.sort();
  _w->_palette->insertStringList(palList);
  int i;
  for (i = 0; i < _w->_palette->count(); ++i) {
    if (_w->_palette->text(i) == ip->paletteName()) {
      break;
    }
  }
  if (i == _w->_palette->count()) {
    i = 0;
  }

  _w->_palette->setCurrentItem(i);
...
void KstImageDialogI::fillFieldsForNew() {
  ...
  if (palList.contains("Kst Spectrum 1021")) {
    _w->_palette->setCurrentText("Kst Spectrum 1021");
  } else if (palList.contains("Kst Grayscale 256")) {
    _w->_palette->setCurrentText("Kst Grayscale 256");
  }
  ...
  updateGroups();
  updateEnables();
  updatePalette(_w->_palette->currentText());
  adjustSize();
  resize(minimumSizeHint());
  setFixedHeight(height());
}
...
bool KstImageDialogI::newObject() {
  ...
  if (_w->_contourOnly->isChecked()) {
    ...
    KPalette* newPal = new KPalette(_w->_palette->currentText());
...
bool KstImageDialogI::editSingleObject(KstImagePtr imPtr) {
  ...
  if (_contourOnlyDirty || _colorOnlyDirty || _colorAndContourDirty) {
    ...
    if (imPtr->hasContourMap() && !imPtr->hasColorMap()) {
      ...
      KPalette *palette;
      if (_paletteDirty) {
        palette = new KPalette(_w->_palette->currentText());
      } else {
...
bool KstImageDialogI::editObject() {
  ...
  if (_editMultipleMode) {
    ...
    _paletteDirty = _w->_palette->currentItem() != 0;
    ...
    if (!ip || (tag_name != ip->tagName() && KST::dataTagNameNotUnique(tag_name))) {
      _tagName->setFocus();
      return false;
    }
...
void KstImageDialogI::updatePalette(const QString& palette) {
...
void KstImageDialogI::populateEditMultiple() {
  ...
  _w->_palette->insertItem("", 0);
  _w->_palette->setCurrentItem(0);
