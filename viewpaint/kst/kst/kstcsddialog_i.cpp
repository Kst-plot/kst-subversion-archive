...
bool KstCsdDialogI::newObject() {
  ...
  int apodizeFxn = _w->_kstFFTOptions->ApodizeFxn->currentItem();
  ...
  KstCSDPtr csd = new KstCSD(tag_name, p, freq, average, removeMean,
                             apodize, apodizeFxn, windowSize, length, gaussianSigma,
...
bool KstCsdDialogI::editSingleObject(KstCSDPtr csPtr) {
  ...
  if (_apodizeFxnDirty) {
    csPtr->setApodizeFxn(_w->_kstFFTOptions->ApodizeFxn->currentItem());
  }
...
bool KstCsdDialogI::editObject() {
  ...
  if (_editMultipleMode) {
    ...
    QString tag_name = _tagName->text();
    if (!cp || (tag_name != cp->tagName() && KST::dataTagNameNotUnique(tag_name))) {
      _tagName->setFocus();
      return false;
    }
