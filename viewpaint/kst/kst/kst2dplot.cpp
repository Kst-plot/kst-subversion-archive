...
void Kst2DPlot::convertTimeValueToString(KstAxisInterpretation axisInterpretation, KstAxisDisplay axisDisplay, QString& label, uint& length, double z, bool isLog) {
  double value;
...
void Kst2DPlot::genAxisTickLabelFullPrecision(KstAxisInterpretation axisInterpretation, KstAxisDisplay axisDisplay, QString& label, uint& length, double z, bool isLog, bool isInterpreted) {
  if (isInterpreted) {
    convertTimeValueToString(axisInterpretation, axisDisplay, label, length, z, isLog);
  } else if (isLog) {
...
void Kst2DPlot::genAxisTickLabels(TickParameters &tp,
  ...
  setTicks(tp.tick, tp.org, Max*scale, Min*scale, isLog, isX, base);
  tp.tick /= scale;
  tp.org  /= scale;
  ...
  for (int i = tp.iLo; i < tp.iHi; i++) {
    genAxisTickLabel(strTmp, (double)i * tp.tick + tp.org, isLog);
    ...
    genAxisTickLabelFullPrecision(axisInterpretation, axisDisplay, strTmp, length, (double)i * tp.tick + tp.org, isLog, isInterpreted);
  ...
  if ((isX && _xTransformed) || (!isX && _yTransformed)) {
    for (int i = tp.iLo; i < tp.iHi; i++) {
      ...
      if (isLog) {
        originalNumber = pow(10, originalNumber);
      }
  ...
  if (bDuplicate || isInterpreted || isOffsetMode) {
    ...
    for (int i = tp.iLo; i < tp.iHi; i++) {
      if (i == iShort) {
        genAxisTickLabelFullPrecision(axisInterpretation, axisDisplay, strTmp, length, (double)iShort * tp.tick + tp.org, isLog, isInterpreted);
        if (!strUnits.isEmpty()) {
...
void Kst2DPlot::setBorders(double& xleft_bdr_px, double& xright_bdr_px,
  ...
  getLScale(x_min, y_min, x_max, y_max);
  genAxisTickLabels(tpx, x_min, x_max, _xLog, _xAxisInterpretation, _xAxisDisplay, true, _isXAxisInterpreted, _xOffsetMode);
  ...
  genAxisTickLabels(tpy, y_min, y_max, _yLog, _yAxisInterpretation, _yAxisDisplay, false, _isYAxisInterpreted, _yOffsetMode);
...
void Kst2DPlot::paintSelf(KstPainter& p, const QRegion& bounds) {
  if (p.type() == KstPainter::P_EXPORT || p.type() == KstPainter::P_PRINT) {
    ...
    if (!wasDirty) {
      ...
      } else if (p.type() == KstPainter::P_ZOOM) {
    ...
    if (doDraw) {
      // FIXME: setting the children (owned labels) as dirty ensures that they get re-parsed
      // when curves change.  This will mean that fit labels get updated.  BUT:
      // this is an inadequate hack which only works if the scalars change when the
      // curves change.  Labels need to dirtied based on changes on scalars they
      // use, regardless of what curves in the plots are doing.
      forEachChild2<KstViewLabel>(&KstViewObject::setDirty);
      draw();
      setDirty(false);
    }
...
void Kst2DPlot::draw(KstPainter& p, double resolutionEnhancement) {
  if (_zoomPaused) {
    return;
  }
  ...
  if (x_px - xright_bdr_px - xleft_bdr_px >= 10.0 &&
    ...
    plotPlotMarkers(p, m_X, b_X, x_max, x_min, y_px, ytop_bdr_px, ybot_bdr_px, penWidth);
    if (_xReversed) {
      p.scale(-1,1);
      p.translate(d2i(-1 * Hx - Lx), 0);
    }
...
void Kst2DPlot::editVector(int id) {
  KstDialogs::showVectorDialog(_objectEditMap[id]);
}
      ...
      if (found) {
        ...
        if (_isXAxisInterpreted) {
          ...
          if (_cursorOffset) {
            if (isXLog()) {
              genOffsetLabel(_xAxisInterpretation, _xAxisDisplay, msgXOffset,
                    _cursor_x, log10(xpos), xmin, xmax, isXLog(), _isXAxisInterpreted);
            } else {
              genOffsetLabel(_xAxisInterpretation, _xAxisDisplay, msgXOffset,
                    _cursor_x, xpos, xmin, xmax, isXLog(), _isXAxisInterpreted);
            }
        ...
        if (_isYAxisInterpreted) {
          ...
          if (_cursorOffset) {
            if (isYLog()) {
              genOffsetLabel(_yAxisInterpretation, _yAxisDisplay, msgYOffset,
                    _cursor_y, log10(ypos), ymin, ymax, isYLog(), _isYAxisInterpreted);
            } else {
              genOffsetLabel(_yAxisInterpretation, _yAxisDisplay, msgYOffset,
                    _cursor_y, ypos, ymin, ymax, isYLog(), _isYAxisInterpreted);
            }
...
void Kst2DPlot::xZoomMax(KstViewWidget *view) {
  ...
  updateTiedPlots(&Kst2DPlot::setXScaleMode, _xScaleModeDefault);
  pushScale();
  setDirty();
}
...
void Kst2DPlot::yZoomMax(KstViewWidget *view) {
  ...
  updateTiedPlots(&Kst2DPlot::setYScaleMode, _yScaleModeDefault);
  pushScale();
  setDirty();
}
...
void Kst2DPlot::yZoomLocalMax(KstViewWidget *view) {
  ...
  updateTiedPlots(&Kst2DPlot::zoomSelfYLocalMax, true);
  pushScale();
  setDirty();
}
  ...
  switch (e->key()) {
    ...
    case Key_Insert:
      if (!e->isAutoRepeat() && GetPlotRegion().contains(cursorPos)) {
        ...
        if (_xLog) {
          setPlotMarker(pow(10,((fakeCursorPos - position().x() - _b_X) / _m_X)), false, false);
        } else {
          setPlotMarker((fakeCursorPos - position().x() - _b_X) / _m_X, false, false);
        }
        setDirty();
      }
      break;
...
void Kst2DPlot::editMatrix(int id) {
  KstDialogs::showMatrixDialog(_objectEditMap[id]);
}
...
void Kst2DPlot::plotPlotMarkers(QPainter& p,
  ...
  p.setPen(QPen(foregroundColor(), penWidth, Qt::DashLine));
