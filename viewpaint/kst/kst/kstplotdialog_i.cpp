...
void KstPlotDialogI::update(int new_index) {
  ...
  if (i_plot >= 0 && c) {
    ...
    if (vl) {
      LegendFontComboBox->setCurrentFont(vl->fontName());
      LegendFontSize->setValue(vl->fontSize());
...
void KstPlotDialogI::applyAppearance(Kst2DPlotPtr plot) {
  ...
  for (uint i = 0; i < plots.size(); i++) {
    ...
    if (ShowLegend->isChecked()) {
      ...
      vl->setFontName(LegendFontComboBox->currentText());
      vl->setFontSize(LegendFontSize->value());
      vl->setVertical(checkBoxVerticalLayout->isChecked());
      vl->setForegroundColor(legendColors->foreground());
      vl->setBackgroundColor(legendColors->background());
...
void KstPlotDialogI::applyPlotMarkers(Kst2DPlotPtr plot) {
  ...
  for (i = 0; i < plots.size(); i++) {
    ...
    if (UseCurve->isChecked()) {
      ...
      KstVCurveList::iterator curves_iter = vcurves.findTag(CurveCombo->currentText());
