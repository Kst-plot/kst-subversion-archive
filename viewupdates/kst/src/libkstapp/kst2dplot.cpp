...
void Kst2DPlot::editVector(int id) {
  KstDialogs::self()->showVectorDialog(_objectEditMap[id]);
}
  ...
  switch (e->key()) {
    ...
    case Key_Insert:
      if (!e->isAutoRepeat() && GetPlotRegion().contains(cursorPos)) {
        ...
        if (_xLog) {
          setPlotMarker(pow(_xLogBase, ((fakeCursorPos - position().x() - _b_X) / _m_X)), false, false);
        } else {
...
void Kst2DPlot::editMatrix(int id) {
  KstDialogs::self()->showMatrixDialog(_objectEditMap[id]);
}
