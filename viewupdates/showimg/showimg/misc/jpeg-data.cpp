/* jpeg-data.c
 *
 * Copyright © 2001-2005 Lutz Müller <lutz@users.sourceforge.net>
 *
...
jpeg_data_new (void)
{
	...
	data = (JPEGData *)malloc (sizeof (JPEGData));
	if (!data)
		return (NULL);
	memset (data, 0, sizeof (JPEGData));
	data->priv = (JPEGDataPrivate*)malloc (sizeof (JPEGDataPrivate));
	if (!data->priv) {
		free (data);
		return (NULL);
	}
...
jpeg_data_append_section (JPEGData *data)
{
	...
	if (!data->count)
		s = (JPEGSection*)malloc (sizeof (JPEGSection));
	else
		s = (JPEGSection*)realloc (data->sections,
...
*/
		...
		/* Write the marker */
		*d = (unsigned char*)realloc (*d, sizeof (char) * (*ds + 2));
		(*d)[*ds + 0] = 0xff;
		(*d)[*ds + 1] = s.marker;
		*ds += 2;
		...
		case JPEG_MARKER_APP1:
			...
			*d = (unsigned char*)realloc (*d, sizeof (char) * (*ds + 2));
			...
			(*d)[*ds + 1] = (eds + 2) >> 0;
			*ds += 2;
			*d = (unsigned char*)realloc (*d, sizeof (char) * (*ds + eds));
			memcpy (*d + *ds, ed, eds);
			*ds += eds;
			free (ed);
			break;
		default:
			*d = (unsigned char*)realloc (*d, sizeof (char) *
		...
		default:
			...
			default:
				...
				s->content.generic.data =
						(unsigned char*)malloc (sizeof (char) * len);
...
jpeg_data_load_file (JPEGData *data, const char *path)
{
	...
	size = ftell (f);
	fseek (f, 0, SEEK_SET);
	d = (unsigned char*)malloc (sizeof (char) * size);
	if (!d) {
		fclose (f);
		return;
	}
