...
// for translatable strings. KLocalizedStrings are produced by QObject::tr* calls,
...
// xxx K_GLOBAL_STATIC(KCmdLineArgsStatic, s)
...
#ifdef Q_WS_X11
    qt_options.add("display <displayname>", QObject::tr("Use the X-server display 'displayname'"));
#elif defined(Q_WS_QWS)
    qt_options.add("display <displayname>", QObject::tr("Use the QWS display 'displayname'"));
#else
#endif
    qt_options.add("session <sessionId>", QObject::tr("Restore the application for the given 'sessionId'"));
    qt_options.add("cmap", QObject::tr("Causes the application to install a private color\nmap on an 8-bit display"));
    qt_options.add("ncols <count>", QObject::tr("Limits the number of colors allocated in the color\ncube on an 8-bit display, if the application is\nusing the QApplication::ManyColor color\nspecification"));
    qt_options.add("nograb", QObject::tr("tells Qt to never grab the mouse or the keyboard"));
    qt_options.add("dograb", QObject::tr("running under a debugger can cause an implicit\n-nograb, use -dograb to override"));
    qt_options.add("sync", QObject::tr("switches to synchronous mode for debugging"));
    qt_options.add("fn");
    qt_options.add("font <fontname>", QObject::tr("defines the application font"));
    qt_options.add("bg");
    qt_options.add("background <color>", QObject::tr("sets the default background color and an\napplication palette (light and dark shades are\ncalculated)"));
    qt_options.add("fg");
    qt_options.add("foreground <color>", QObject::tr("sets the default foreground color"));
    qt_options.add("btn");
    qt_options.add("button <color>", QObject::tr("sets the default button color"));
    qt_options.add("name <name>", QObject::tr("sets the application name"));
    qt_options.add("title <title>", QObject::tr("sets the application title (caption)"));
#ifdef Q_WS_X11
    qt_options.add("visual TrueColor", QObject::tr("forces the application to use a TrueColor visual on\nan 8-bit display"));
    qt_options.add("inputstyle <inputstyle>", QObject::tr("sets XIM (X Input Method) input style. Possible\nvalues are onthespot, overthespot, offthespot and\nroot"));
    qt_options.add("im <XIM server>", QObject::tr("set XIM server"));
    qt_options.add("noxim", QObject::tr("disable XIM"));
#endif
#ifdef Q_WS_QWS
    qt_options.add("qws", QObject::tr("forces the application to run as QWS Server"));
#endif
    qt_options.add("reverse", QObject::tr("mirrors the whole layout of widgets"));
    qt_options.add("stylesheet <file.qss>", QObject::tr("applies the Qt stylesheet to the application widgets"));
    qt_options.add("graphicssystem <system>", QObject::tr("use a different graphics system instead of the default one, options are raster and opengl (experimental)"));
    // KDE options
//     kde_options.add("caption <caption>",   QObject::tr("Use 'caption' as name in the titlebar"));
//     kde_options.add("icon <icon>",         QObject::tr("Use 'icon' as the application icon"));
//     kde_options.add("config <filename>",   QObject::tr("Use alternative configuration file"));
//     kde_options.add("nocrashhandler",      QObject::tr("Disable crash handler, to get core dumps"));
// #ifdef Q_WS_X11
//     kde_options.add("waitforwm",           QObject::tr("Waits for a WM_NET compatible windowmanager"));
// #endif
//     kde_options.add("style <style>",       QObject::tr("sets the application GUI style"));
//     kde_options.add("geometry <geometry>", QObject::tr("sets the client geometry of the main widget - see man X for the argument format (usually WidthxHeight+XPos+YPos)"));
// #ifndef Q_WS_WIN
...
public:
    KCmdLineArgsPrivate(const KCmdLineOptions &_options, const QString &_name, const QByteArray &_id)
        : options(_options)
        , name(_name)
        , id(_id)
        , parsedOptionList(0)
...
{
   init(_argc, _argv,
        new KAboutData(_appname, 0, QObject::tr(_appname), "unknown", QObject::tr("KDE Application")));
...
void KCmdLineArgs::addStdCmdLineOptions(StdCmdLineArgs stdargs) {
   if (stdargs & KCmdLineArgs::CmdLineArgQt) {
       KCmdLineArgs::addCmdLineOptions(s->qt_options, QObject::tr("Qt"), "qt");
   }
   if (stdargs & KCmdLineArgs::CmdLineArgKDE) {
       KCmdLineArgs::addCmdLineOptions(s->kde_options, QObject::tr("KDE"), "kde");
   }
   s->mStdargs = stdargs;
}

void
KCmdLineArgs::addCmdLineOptions( const KCmdLineOptions &options, const QString &name,
...
KCmdLineArgs::loadAppArgs( QDataStream &ds)
{
   ...
   while(count--)
   {
     ...
     if (!found) {
// xxx         kWarning() << "Argument definitions for" << id << "not found!";
                               ...
                               int &i, bool _enabled, bool &moreOptions)
{
   ...
   if (args == s->argsList->end() || !result)
   {
      ...
      KCmdLineArgs::usageError( QObject::tr("Unknown option '%1'.", QString::fromLocal8Bit(_opt)));
   }
   ...
   if (result == 3) // This option takes an argument
   {
      ...
      if (argument.isEmpty())
      {
         ...
         {
            KCmdLineArgs::enable_i18n();
            KCmdLineArgs::usageError( QObject::tr("'%1' missing.",  QString::fromLocal8Bit(opt_name)), "@info:shell %1 is cmdoption name");
         }
         argument = s->argv[i];
      }
...
KCmdLineArgsStatic::parseAllArgs()
{
   ...
   for(int i = 1; i < s->argc; i++)
   {
      ...
      if ((s->argv[i][0] == '-') && s->argv[i][1] && inOptions)
      {
         ...
         {
            KCmdLineArgs::enable_i18n();
            s->printQ(QObject::tr("Qt: %1\n"
                            "KDE Development Platform: %2\n"
                            "%3: %4\n",
                            QString::fromLatin1(qVersion()),
                            //QString::fromLatin1(KDE_VERSION_STRING),
                            s->about->programName(), s->about->version()), "@info:shell message on appcmd --version; do not translate 'Development Platform'"
       ...
       if ( s->about ) {
         ...
         if ( !authors.isEmpty() ) {
           ...
           s->printQ( QObject::tr(QString(s->about->programName()) ,  authorlist ), "the 2nd argument is a list of name+address, one on each line","%1 was written by\n%2" );
         }
       } else {
      ...
      else
      {
         ...
         if (!allowArgs)
         {
            if (s->ignoreUnknown)
               continue;
            KCmdLineArgs::enable_i18n();
            KCmdLineArgs::usageError(QObject::tr("Unexpected argument '%1'.", KuitSemantics::escape(s->decodeInput(s->argv[i]))));
         }
         else
         {
     ...
     while (args != s->argsList->end())
     {
       ...
       for (int i = 0; i < option.d->names.size(); i++)
       {
         ...
         if (!option.d->descriptions[i].isEmpty()) {
            descriptionFull = option.d->descriptions[i];
         }
...
KCmdLineArgs::addTempFileOption()
{
    ...
    tmpopt.add( "tempfile", QObject::tr("The files/URLs opened by the application will be deleted after use") );
    KCmdLineArgs::addCmdLineOptions( tmpopt, QObject::tr("KDE-tempfile"), "kde-tempfile" );
}
