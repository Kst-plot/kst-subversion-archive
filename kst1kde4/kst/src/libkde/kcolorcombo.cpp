...
static QBasicAtomicPointer< QVector<QColor> > _k_static_standardPalette = Q_BASIC_ATOMIC_INITIALIZER(0);
static bool _k_static_standardPalette_destroyed;                                      
static struct _k_standardPalette                              
{                                                                              
    bool isDestroyed()                                                         
    {                                                                          
        return _k_static_standardPalette_destroyed;                                   
    }                                                                          

    inline operator QVector<QColor>*()                                                    
    {                                                                          
        return operator->();                                                   
    }                                                                          

    inline QVector<QColor> *operator->()                                                  
    {                                                                          
        if (!_k_static_standardPalette) {                                               
            if (isDestroyed()) {                                               
                qFatal("Fatal Error: Accessed global static '%s *%s()' after destruction. " 
                       "Defined at %s:%d", "QVector<QColor>", "standardPalette", __FILE__, __LINE__);  
            }                                                                  
            QVector<QColor> *x = new QVector<QColor>(STANDARD_PAL_SIZE);                                           
            if (!_k_static_standardPalette.testAndSetOrdered(0, x)                      
                && _k_static_standardPalette != x ) {                                   
                delete x;                                                      
            } else {                                                           
                static KCleanUpGlobalStatic cleanUpObject = { destroy };       
            }                                                                  
        }                                                                      
        return _k_static_standardPalette;                                               
    }                                                                          
    ...
    static void destroy()                                                      
    {                                                                          
        _k_static_standardPalette_destroyed = true;                                   
        QVector<QColor> *x = _k_static_standardPalette;                                            
        _k_static_standardPalette = 0;                                                  
        delete x;                                                              
    }                                                                          
} standardPalette;
...
{
    if (index == 0) {
        if (QColorDialog::getColor(customColor, q) == QDialog::Accepted) {
