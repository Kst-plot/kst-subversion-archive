...
void KstJS::addScript(QString script) {
...
void KstJS::doArgs() {
  ...
  for (QStringList::ConstIterator i = a.begin(); i != a.end(); ++i) {
    rc |= _jsPart->execute(*i);
  }
  executing = false;
  if (rc) {
    QTimer::singleShot(0, this, SLOT(showConsole()));
  }
}
...
bool KstJS::writeHistory(const QString &filename) {
  bool retVal = false;

  if (_iface) {
    retVal = _iface->writeHistory(filename);
  }
...
bool KstJS::addToOutput(const QString &output) {
  bool retVal = false;

  if (_iface) {
    retVal = _iface->addToOutput(output);
  }
