...
static ViewObjectProperties viewObjectProperties[] = {
  ...
  { "maximized", &KstBindViewObject::setMaximized, &KstBindViewObject::maximized },
...
void KstBindViewObject::setMaximized(KJS::ExecState *exec, const KJS::Value& value) {
  ...
  if (d) {
    ...
    bool maximized = value.toBoolean(exec);
    if (maximized) {
      KstViewObjectPtr tlv = d->topLevelParent();
      if (tlv) {
        tlv->recursively(&KstViewObject::setMaximized, false);
      }
    }
...
KJS::Value KstBindViewObject::remove(KJS::ExecState *exec, const KJS::List& args) {
...

  {
    ...
    if (tlvTest) {
      return createGeneralError(exec, i18n("Unable to delete the view."));
    }
