...
void doTests() {
  ...
  v1->zero();
  for (int i = 0; i < 15; ++i) {
    doTest(v1->value()[i] == 0.0);
  }
  doTest(!v1->isScalarList());
  v1->value()[0] = -42.0;
  v1->resize(3);
  doTest(v1->length() == 3);
  doTest(v1->value()[0] == -42.0);
  ...
  data[1] = 2.0;
  data[2] = 3.0;
  doTest(v2->interpolate(0, 5) == 1.0);
  doTest(v2->interpolate(1, 5) == 1.5);
  doTest(v2->interpolate(2, 5) == 2.0);
  doTest(v2->interpolate(3, 5) == 2.5);
  doTest(v2->interpolate(4, 5) == 3.0);
}
...
int main(int argc, char **argv) {
  ...
  KCmdLineArgs::init(argc, argv, "testvector", "testvector", "testvector", "1.0.0.0");
