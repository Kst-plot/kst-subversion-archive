...
void doTests() {
  ...
  am2->change(am2->tag(), 3, 3, 0, 0, 0, 0); //should not be legal
  doTest(am2->xNumSteps() == 3);
  doTest(am2->yNumSteps() == 3);
  doTest(am2->minX() == 0);
  doTest(am2->minY() == 0);
  ...
  KstAMatrix* um1 = new KstAMatrix(KstObjectTag("Unity"), 3, 3, 0.0, 0.0, 1.0, 1.0);
  ...
  KstAMatrix* sm = new KstAMatrix(KstObjectTag("Spike"), 2, 2, 0.0, 0.0, 1.0, 1.0);
...
int main(int argc, char **argv) {
  ...
  KCmdLineArgs::init(argc, argv, "testamatrix", "testamatrix", "testamatrix", "1.0.0.0");
