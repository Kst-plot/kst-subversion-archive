...
QDomDocument makeDOM1(const QString& tag, const QString& val, bool isGlobal = false) {
  ...
  if (isGlobal) {
    child = loadTest.createElement("isGlobal");
    scalardoc.appendChild(child);
  }
...
void doTests() {
  ...
  *sp = 1.1415;
  ...
  KstScalarPtr sp2 = new KstScalar(sp->tag());
  doTest(sp2->tagName() == sp->tagName() + "-1");
...
int main(int argc, char **argv) {
  ...
  KCmdLineArgs::init(argc, argv, "testscalars", "testscalars", "testscalars", "1.0.0.0");
