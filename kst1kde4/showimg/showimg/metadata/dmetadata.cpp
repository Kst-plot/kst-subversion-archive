...
DMetadata::DMetadata(const QString& filePath, const QString& ff/*="NONE"*/)
{
    d = new DMetadataPriv;
    load(filePath, ff);
}
...
QByteArray DMetadata::getExif() const
{
    ...
    catch( Exiv2::Error &e )
    {
        if (!d->filePath.isEmpty())
            MYDEBUG << "From file " << d->filePath << endl;

        MYDEBUG << "Cannot get Exif data using Exiv2 (" 
...
QByteArray DMetadata::getIptc(bool addIrbHeader) const
{
    ...
    {    
        ...
        {                
            ...
            if (addIrbHeader) 
            {
                ...
                {
                    MYDEBUG << "Exiv2 version is to old. Cannot add Irb header to IPTC metadata" << endl;
                    return QByteArray();
                }
#endif
            }
    ...
    catch( Exiv2::Error &e )
    {
        ...
        MYDEBUG << "Cannot get Iptc data using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }       
...
void DMetadata::setExif(const QByteArray& data)
{
    ...
    catch( Exiv2::Error &e )
    {
        ...
        MYDEBUG << "Cannot set Exif data using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
}
...
void DMetadata::setIptc(const QByteArray& data)
{
    ...
    catch( Exiv2::Error &e )
    {
        ...
        MYDEBUG << "Cannot set Iptc data using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
}
...
bool DMetadata::load(const QString& filePath, const QString& ff)
{
    QString format = ff;
...
bool DMetadata::save(const QString& filePath, const QString& ff)
{
    {
        if(ff=="JPEG")
        {
...
QImage DMetadata::getExifThumbnail(bool fixOrientation) const
{
        ...
        if (!thumbnail.isNull())
        {
            if (fixOrientation)
            {
                ...
                if (it != exifData.end())
                {
                    ...
                    MYDEBUG << " Exif Thumbnail Orientation: " << orientation << endl;
    ...
    {
        MYDEBUG << "Cannot get Exif Thumbnail using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setExifThumbnail(const QImage& thumb)
{
    try
    {   
        KTempFile thumbFile(QString::null, "ShowImgDMetadataThumb");
    ...
    {
        MYDEBUG << "Cannot set Exif Thumbnail using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
DMetadata::ImageColorWorkSpace DMetadata::getImageColorWorkSpace()
{
    ...
    {
        MYDEBUG << "Cannot parse image color workspace tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImageColorWorkSpace(ImageColorWorkSpace workspace)
{
    ...
    {    
        ...
        MYDEBUG << "Exif color workspace tag set to: " << workspace << endl;
        return true;
    }
    catch( Exiv2::Error &e )
    {
        MYDEBUG << "Cannot set Exif color workspace tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
QSize DMetadata::getImageDimensions()
{
    ...
    {
        MYDEBUG << "Cannot parse image dimensions tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImageDimensions(const QSize& size)
{
    ...
    {
        MYDEBUG << "Cannot set Date & Time into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
DMetadata::ImageOrientation DMetadata::getImageOrientation()
{
    ...
    try
    {
        ...
        if (supportMinolta)
        {
            ...
            if (it != exifData.end())
            {
                ...
                MYDEBUG << "Minolta Makernote Orientation: " << orientation << endl;
                switch(orientation)
                {
                    case 76:
        ...
        {
            orientation = it->toLong();
#if DMETADATA_DEBUG > 0
            MYDEBUG << "Exif Orientation: " << orientation << endl;
#endif
    ...
    {
        MYDEBUG << "Cannot parse Exif Orientation tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
bool DMetadata::setImageOrientation(ImageOrientation orientation)
{
    ...
    {    
        ...
        {
            MYDEBUG << k_funcinfo << "Exif orientation tag value is not correct!" << endl;
            return false;
        }
        
        d->exifMetadata["Exif.Image.Orientation"] = (uint16_t)orientation;
        MYDEBUG << "Exif orientation tag set to: " << orientation << endl;
        ...
        if (supportMinolta)
        {
            ...
            if (it != d->exifMetadata.end())
            {
                ...
                MYDEBUG << "Removing Exif.MinoltaCs7D.Rotation tag" << endl;
            }
            ...
            {
                d->exifMetadata.erase(it);
                MYDEBUG << "Removing Exif.MinoltaCs5D.Rotation tag" << endl;
            }
        }
    ...
    {
        MYDEBUG << "Cannot set Exif Orientation tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
QDateTime DMetadata::getImageDateTime() const
{
    ...
    {    
        ...
        {        
            ...
            if (it != exifData.end())
            {
                ...
                if (dateTime.isValid())
                {
                    // MYDEBUG << "DateTime (Exif standard): " << dateTime << endl;
                    return dateTime;
                }
            }
            ...
            if (it2 != exifData.end())
            {
                ...
                if (dateTime.isValid())
                {
                    // MYDEBUG << "DateTime (Exif original): " << dateTime << endl;
                    return dateTime;
                }
            }
            ...
            if (it3 != exifData.end())
            {
                ...
                if (dateTime.isValid())
                {
                    // MYDEBUG << "DateTime (Exif digitalized): " << dateTime << endl;
                    return dateTime;
                }
            }
        }
        ...
        {        
            ...
            if (it != iptcData.end())
            {
                ...
                if (it2 != iptcData.end())
                {
                    ...
                    if (dateTime.isValid())
                    {
                        // MYDEBUG << "Date (IPTC created): " << dateTime << endl;
                        return dateTime;
                    }                    
                }
            }                        
            ...
            if (it3 != iptcData.end())
            {
                ...
                if (it4 != iptcData.end())
                {
                    ...
                    if (dateTime.isValid())
                    {
                        // MYDEBUG << "Date (IPTC digitalized): " << dateTime << endl;
                        return dateTime;
                    }                    
                }
            }                       
    ...
    {
        MYDEBUG << "Cannot parse Exif date & time tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
QString DMetadata::getImageComment() const
{
    ...
    {
        MYDEBUG << "Cannot get Image comments using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
bool DMetadata::setImageComment(const QString& comment)
{
    try
    {
        ...
        MYDEBUG << d->filePath << " ==> Comment: " << comment << endl;
    ...
    {
        MYDEBUG << "Cannot set Comment into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
QString DMetadata::convertCommentValue(const Exiv2::Exifdatum &exifDatum)
{
    ...
    {
        MYDEBUG << "Cannot convert Comment using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
/*
Iptc.Application2.Urgency <==> ShowImg Rating links:
...
int DMetadata::getImageRating() const
{
    ...
    {
        MYDEBUG << "Cannot get Image Rating tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImageRating(int rating)
{
    ...
    {    
        ...
        {
            MYDEBUG << k_funcinfo << "Rating value to write out of range!" << endl;
            return false;
        }

        MYDEBUG << d->filePath << " ==> Rating: " << rating << endl;
    ...
    {
        MYDEBUG << "Cannot set Rating tag into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
QStringList DMetadata::getImageKeywords() const
{
    ...
    {
        MYDEBUG << "Cannot get Keywords from image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImageKeywords(const QStringList& oldKeywords, const QStringList& newKeywords)
{
    ...
    {    
        ...
        setImageProgramId();
        MYDEBUG << d->filePath << " ==> Keywords: " << newkeys << endl;
    ...
    {
        MYDEBUG << "Cannot set Keywords into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImagePhotographerId(const QString& author, const QString& authorTitle)
{
    ...
    {    
        ...
        Byline.truncate(32);
        MYDEBUG << d->filePath << " ==> Author: " << Byline << endl;
        ...
        MYDEBUG << d->filePath << " ==> Author Title: " << BylineTitle << endl;
    ...
    {
        MYDEBUG << "Cannot set Photographer identity into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImageCredits(const QString& credit, const QString& source, const QString& copyright)
{
    ...
    {    
        ...
        MYDEBUG << d->filePath << " ==> Copyright: " << Copyright << endl;
    ...
    {
        MYDEBUG << "Cannot set Credits identity into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setImageProgramId()
{
    try
    {
        QString software("showimg-");
        //software.append(showimg_version);
        d->exifMetadata["Exif.Image.Software"] = software.ascii();

        d->iptcMetadata["Iptc.Application2.Program"] = "showimg";
        d->iptcMetadata["Iptc.Application2.ProgramVersion"] = "0.0.1";//showimg_version;
        return true;
    }
    catch( Exiv2::Error &e )
    {
        MYDEBUG << "Cannot set Program identity into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
PhotoInfoContainer DMetadata::getPhotographInformations() const
{
    ...
    {
        MYDEBUG << "Cannot get Photograph informations into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
bool DMetadata::setExifTagString(const char * exifTagName, const QString& value)
{
    ...
    {
        MYDEBUG << "Cannot set Exif tag string into image using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
bool DMetadata::getImagePreview(QImage& preview)
{
    ...
    {
        MYDEBUG << "Cannot get image preview using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }
...
bool DMetadata::setImagePreview(const QImage& preview)
{
    try
    {
        KTempFile previewFile(QString::null, "ShowImgDMetadataPreview");
        ...
        MYDEBUG << "(" << preview.width() << "x" << preview.height() 
...
bool DMetadata::getGPSInfo(double& altitude, double& latitude, double& longitude)
{
    ...
    {
        MYDEBUG << "Cannot get Exif GPS tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::setGPSInfo(double altitude, double latitude, double longitude)
{
    ...
    {
        MYDEBUG << "Cannot set Exif GPS tag using Exiv2 (" 
                  //<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
...
bool DMetadata::removeGPSInfo()
{
    ...
    {
        MYDEBUG << "Cannot remove Exif GPS tag using Exiv2 (" 
                  ////<< QString::fromLocal8Bit(e.what().c_str())
                  << ")" << endl;
    }        
