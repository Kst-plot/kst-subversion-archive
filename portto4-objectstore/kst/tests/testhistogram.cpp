...
void TestHistogram::testHistogram() {
  ...
  Kst::HistogramPtr h1 = new Kst::Histogram(&_store, Kst::ObjectTag::fromString("H1"), vp, 0, 10, 10, Kst::KST_HS_NUMBER);
  ...
  QCOMPARE(count, 100); // should still account for the whole vector
  // min > max
  h1 = new Kst::Histogram(&_store, Kst::ObjectTag::fromString("H2"), vp, 10, 0, 10, Kst::KST_HS_NUMBER);
  QCOMPARE(h1->nBins(), 10);
  ...
  QCOMPARE(h1->vNumSamples(), 100);
  // min == max
  h1 = new Kst::Histogram(&_store, Kst::ObjectTag::fromString("H3"), vp, 10, 10, 2, Kst::KST_HS_NUMBER);
  QCOMPARE(h1->nBins(), 2);
  ...
  QCOMPARE(h1->vNumSamples(), 100);
  // max < min
  h1 = new Kst::Histogram(&_store, Kst::ObjectTag::fromString("H4"), vp, 11, 9, 1, Kst::KST_HS_NUMBER);
  QCOMPARE(h1->nBins(), 2);
  QCOMPARE(h1->xMax(), 11.0);
  QCOMPARE(h1->xMin(), 9.0);
  QCOMPARE(h1->vMin(), 0.0);
